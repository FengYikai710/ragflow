...
# RAGFlow Data Migration Script (Vastbase)
# Usage: ./migration_vb.sh [-p project_name] [backup|restore] [backup_folder]
#
# This script helps you backup and restore RAGFlow Docker volumes
# for a Vastbase deployment: MinIO, Redis, and Vastbase data.
# (MySQL and Elasticsearch volumes are intentionally excluded; back them up
#  separately if your deployment uses them.)
...
VOLUME_BASES=("minio_data" "redis_data" "vastbase_data")
BACKUP_FILES=("minio_backup.tar.gz" "redis_backup.tar.gz" "vastbase_backup.tar.gz")
...
show_help() {
    echo "RAGFlow Data Migration Tool (Vastbase)"
    echo ""
    echo "USAGE:"
    echo "  $0 [-p project_name] <operation> [backup_folder]"
    echo ""
    echo "OPERATIONS:"
    echo "  backup   - Create backup of RAGFlow data volumes"
    ...
    echo "  - ${DEFAULT_PROJECT_NAME}_vastbase_data  (Vastbase database)"
    echo ""
    echo "NOTE:"
